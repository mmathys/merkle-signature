...
func NewTree(hashes [][]byte) (*MerkleTree, error) {
	var defaultHashStrategy = sha256.New
	t := &MerkleTree{
		hashStrategy: defaultHashStrategy,
	}
	root, leafs, err := buildWithContent(hashes, t)
	if err != nil {
		return nil, err
	}
	t.Root = root
...
func NewTreeWithHashStrategy(hashes [][]byte, hashStrategy func() hash.Hash) (*MerkleTree, error) {
	t := &MerkleTree{
...
func (m *MerkleTree) GetMerklePath(hash []byte) ([][]byte, []int64, error) {
	for _, current := range m.Leafs {
		ok := bytes.Equal(current.Hash, hash)
...
func buildWithContent(hashes [][]byte, t *MerkleTree) (*Node, []*Node, error) {
	if len(hashes) == 0 {
...
func (m *MerkleTree) RebuildTree() error {
	...
	root, leafs, err := buildWithContent(hashes, m)
	if err != nil {
		return err
	}
	m.Root = root
...
func (m *MerkleTree) RebuildTreeWith(hashes [][]byte) error {
	root, leafs, err := buildWithContent(hashes, m)
	if err != nil {
		return err
	}
